...
在启用了 DNSSEC 的域名，Whois 记录会显示 DNSSEC 的状态。DS（Delegation Signer）记录保存在上级域名服务器中，它显示了公钥的 Hash 值。  
每次查询都会返回一个 RRSIG 记录，这个记录是自动使用私钥加密后的结果，也就是说只能通过你发布的公钥（即 DNSKEY，发布在本域名的起始记录中）解密，又因为 DNSKEY 的 Hash DS 记录被上级服务器同时传回由上级的私钥签名的 RRSIG 记录，所以 DNS 的信任链就这样建立了。
