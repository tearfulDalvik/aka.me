...
摘要算法是指将一串不定长度的信息通过一定变换，得到一串固定长度的新信息。只要信息和摘要算法不变，摘要结果就一定一致；反之，如果信息出现了任何变化，摘要的变化将会非常明显。摘要算法不可逆，但可以碰撞。**摘要通常用在数字签名、生成树、一致性校验、P2P 信息共享等场景。 常见的摘要算法有 MDx，SHA，RIPEMD，xcrypt 和 HMAC 系列。**   
...
## 加密货币
以加密货币中的老大哥比特币（BTC）为例。在注册比特币钱包时会生成一串私钥，然后通过 ECC 和 `secp256k1` 曲线生成对应的公钥。使用 SHA-256、RIPEMD160 将公钥摘要后通过 base64 除去符号和普通易混淆字母后得到的新字母表 base58 和其他编码算法加上校验合成为比特币钱包地址[^1]。别人就通过这个地址给你打钱。

请注意，地址被摘要过，是无法推回公钥的。即使加了校验，也只能在拥有公钥的情况下验证地址是正确的，不能保证这个地址存在。

比特币的核心是区块链，区块链由无数区块组成，每个区块保存了数千笔交易，由矿工添加到区块链中。

每笔比特币转账都会被记载到区块中，因此比特币交易（不是炒币）的信用体系就通过公开交易记录来实现，所有的交易、区块和地址都能在 [Blockchain.com](https://www.blockchain.com/) 查询到。大家都知道你之前收到过多少钱，花了多少，也就都知道了你的余额。每一笔交易都保存了付款的数量（Value）、对方的地址（Address）、付款者的公钥（Pkscript）等基本信息，并被付款者的私钥签名（Sigscript）。

那别人给你打了钱，在区块链中记录为你的地址，却不能推回公钥，怎么能算成你的呢？其实，从你的第一笔付款（净支出）开始，公钥就会流传在网络中。所有人这时候就知道了你的公钥和地址，通过公钥 Hash 和地址比对后，两者就公开绑定了，之前净收入也就可以算到这把公钥上了。所以如果打钱给了不存在的地址，那这笔钱还真就人间蒸发了。如果别人想自己弄一把自己的公钥绑定到某个地址上是行不通的，因为所有人都会发现这把公钥 Hash 计算后不能得到这个地址。

举个例子：大家都把钱转到你的账户（地址），但却不知道你是谁。别人即使不知道你是谁，但他们也不傻，忽然有个人拿着他自己的银行卡（公钥）出来说这些钱都是我的，人们就会发现他的银行卡（公钥）不能对应上这个银行账户（地址），于是知道他在撒谎。直到有一天你要花钱了，拿着你的银行卡（公钥）去消费，这时候大家一比对，对上了！这时候真相大白：原来账户背后的人是你呀！最后如果大家转给了一个不存在的账户，就永远也没有人拿着对应的银行卡出来消费。

所有的交易记录都公开。比如，地址 [18cBEMRxXHqzWWCxZNtU91F5sbUNKhL5PX](https://www.blockchain.com/btc/address/18cBEMRxXHqzWWCxZNtU91F5sbUNKhL5PX) 一共产生了 24,372 次交易，余额为 $4,101,748.69 美元。

再比如，地址 1CbXpSVmUEjdtDfnHbfkfWuQcDPiLBMkGG 进行的一次 Hash 为 [0f856a211a31fb3a9892ae6d38aeced7e8388531f5eadb79532059ac9be63cfc](https://www.blockchain.com/btc/tx/0f856a211a31fb3a9892ae6d38aeced7e8388531f5eadb79532059ac9be63cfc) 的交易，这笔交易一共转出了 $121.71 美元，其中 $6.87 美元交给矿工用作手续费，$114.84 美元成功到账。在 Inputs 中可以看到由该地址私钥签名的 Sigscript 和该地址的公钥 Pkscript，也同时包含了这笔交易的转出数量和目的地址。

如果上升到区块，那么在区块 [677983](https://www.blockchain.com/btc/block/0000000000000000000861dacbeaeef253591e6e4fc7b7bef37de76e5b91de02) 中，存在 2,603 笔交易，总价值 96941.651 BTC，约 ￥37,388,741,315.12 元人民币。

可以看到，每一笔交易，都可以付给矿工一定的手续费（像不像 VAT？）来加速转账，因为矿工会优先打包有手续费的交易。而矿工除了可以从手续费中赚钱外，每往区块链中附加一个区块也可以得到一笔奖金。只是想附加区块到链上并不容易，矿工需要比别人快、或者运气比别人好。

最后，将交易打包成块并存放到区块链的行为就称为挖矿。只有少数人能够将区块打包到区块链中，因为如果人人都能同时附加区块，那么链中必然会出现信息不一致（同一笔交易会以不同的顺序记录在链中，如此便有恶意操作的空间）。BTC 采用的机制是 PoW（Proof of Work）。PoW 即在打包时，必须通过调整区块头的随机数使得区块的 Hash 的前 n 位数全部为 0。该 n 值随着参与挖矿的人数改变，人越多，n 越大，能够找到前 n 位全部为 0 的随机数就越难。比如，刚才提到的区块 677983 的 Hash 值为 `0000000000000000000861dacbeaeef253591e6e4fc7b7bef37de76e5b91de02`，其前 19 位全部为 0。
