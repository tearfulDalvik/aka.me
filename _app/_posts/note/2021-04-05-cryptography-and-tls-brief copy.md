...
摘要算法是指将一串不定长度的信息通过一定变换，得到一串固定长度的新信息。只要信息和摘要算法不变，摘要结果就一定一致；反之，如果信息出现了任何变化，摘要的变化将会非常明显。摘要算法不可逆，但可以碰撞。**摘要通常用在数字签名、生成树、完整性检验、P2P 信息共享等场景。常见的摘要算法有 MDx、SHA、RIPEMD、xcrypt、CRC 和 HMAC 系列。**   
