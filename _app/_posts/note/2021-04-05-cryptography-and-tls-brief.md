...
当客户端和服务器建立了安全连接之后，浏览器会显示一个小锁在地址栏旁：![]({{ site.file }}/b01839032157c7fd052777df1372388c.png){: .no-lightense .inline}。这时候，你和网页中的所有元素打交道，黑客都是看不到的。如果浏览器检测到了风险，有一些元素没有通过安全的连接发送和接收，浏览器会显示一个醒目的红色感叹号 ![]({{ site.file }}/cca179c52eb7eeb8856dd36627baa652.png){: .no-lightense .inline} 来提醒你。  
...
## 加密、摘要、数字签名和编码
**加密**算法旨在将明文通过一定算法变为不可理解的密文，并能通过一定的算法将密文恢复为原文。其中，在原文相同的情况下，随着提供的密钥不同，得到的密文也不同。**加密通常用来保护明文。常见的加密算法有 AES、RCx、RSA（非对称）、ECC（非对称） 系列。**  

---

**摘要算法**是指将一串不定长度的信息通过一定变换，得到一串固定长度的新信息。只要信息和摘要算法不变，摘要结果就一定一致；反之，如果信息出现了任何变化，摘要的变化将会非常明显。摘要算法不可逆，但可以碰撞。在防碰撞的摘要算法设计中，算法应当更耗时。**摘要通常用在数字签名、生成树、完整性检验、P2P 信息共享、拖时间等场景。常见的摘要算法有 MDx、SHA、RIPEMD、xcrypt、CRC 和 HMAC 系列。** 

---
  
**数字签名**指通过私钥，对另一串不定长度的信息的摘要进行加密，得到一串可供任何拥有公钥的人解密的新字符串。请注意，数字签名必须使用非对称加密。**通常用于信任链。常见的数字签名算法有 ECDSA、RSA、SHA 等系列和他们的组合。** 

---
  
**编码**则指通过一定的变换，使信息从一种表现形式变成另一种表现形式。这样的变化必须存在逆变换，且逆变换所消耗的计算资源和正向变换应该相当。**通常用于将二进制转换成能够复制、便携的字符串，或者在字符串间转换。常见的编码算法有 BaseX、UTF、ASCII 等系列。** 
...
先前的加密函数都只关心一个问题：别人不能访问你的数据，不能对加密数据进行处理，否则会损坏数据，导致解密失败，关注的是数据的存储安全。但是，如果你需要让他人对你的数据进行处理，又不希望他可以访问你的内容，就需要关注数据的处理安全。

同态加密是一种加密函数，关注数据处理安全。它可以让系统在不进行解密的情况下对数据进行处理，即直接对加密数据进行处理，而拥有密钥的用户对处理过的数据进行解密后，得到的正好是处理后的结果。

同态加密对云计算行业非常有用，因为它允许云计算提供商仅仅提供算力，而无法读取敏感的数据，造成隐私泄漏的问题。[Google 曾在 2021 年 6 月开源了自己的全同态加密方案](https://developers.googleblog.com/2021/06/our-latest-updates-on-fully-homomorphic-encryption.html)。

<!-- 如果一个加密函数同时满足加法同态和乘法同态，称为全同态加密。目前可以构造全同态加密的密码学假设主要有理想格上的理想陪集问题(Ideal Coset Problem, ICP)、整数上的近似最大公因子问题(Approximate Greatest Common Devisior, AGCD)、错误学习问题(Learning with Errors, LWE)等等。由于LWE问题尚无有效的量子求解算法, 因此基于LWE假设的加密方案被认为是抗量子的。 -->
...
![](){: .lazy data-src="{{ site.file }}/4661c0cd5e24f1e60aacbd1a6194e245.png"}
...
CSPRNG 是一个安全随机数生成算法，只需向函数提供一个种子（Seed），就可以通过该种子来产生大量的随机数。

实际上，在现代操作系统的实现中，熵不一定随时都够用，真随机数太难获取，因此使用 CSPRNG 算法来生成随机数。即通过真随机数的生成方法生成了一个随机的种子，然后放到 CSPRNG 中为系统提供源源不断的新随机数。

在 Linux 系统中，可以通过读取 `/dev/urandom` 设备来获取源源不断的通过 CSPRNG 算法生成的伪随机数，通过 `/dev/random` 来获得真随机数。显然，伪随机数的读取速度会快很多，而因为种子是使用真随机数生成的，因此该伪随机数也可以理解成安全的。在某些操作系统中（比如 macOS 和 FreeBSD），这两个设备的实现已经是一样的了。
...
![](){: .lazy data-src="{{ site.file }}/5576424a94f5f08f2775ad88d37bef74.png"}
*来源： IETF 8446*
...
![](){: .lazy data-src="{{ site.file }}/2167a420695e96e074a76e1923864f28.png"}
...
最后，加密学的假设是 $$ P \neq NP $$（也可以理解成离散对数难解性）。当今世界瞬息万变，如果有一天人们找到了更好的解题办法，现存的加密方法都将土崩瓦解。关心信息安全的你，也许应该保持一颗时常关注行业信息的心。
